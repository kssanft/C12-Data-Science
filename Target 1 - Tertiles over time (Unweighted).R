...
tertile_growth <- gdp_per_capita2 %>%
  ...
  summarise( 
    average_gdp = mean (GDP_capita)
  ) %>%
  ungroup() %>%
  group_by (Continent, Development) %>%
  arrange(Year) %>%
  mutate(
    average_gdp_growth = (average_gdp - lag(average_gdp)) /
      lag(average_gdp) * 100
  ) %>%
  drop_na()
...
plot_continent <- function(continent_name) {
  ...
  ggplot(tertile_growth, aes(x = Year, y = average_gdp_growth, color = Development, group = Development)) +
    ...
    labs(
      title = paste ("GDP per Capita Growth Rate in", continent_name),
      x = "Year",
...
ggplot(low_growth, aes(x = Year, y = average_gdp_growth, color = Continent, group = Continent)) +
  geom_line(linewidth = 1.25) +
  geom_hline(yintercept = 7, linetype = "dashed", color = "black", linewidth = 1) +
  annotate("text", x = max(low_growth$Year), y = 7, label = "7%", 
           color = "black", vjust = -0.5, hjust = 1.1, size = 4) +
  labs(
    title = "GDP Growth Rate - Low Development",
    x = "Year",
    y = "GDP per Capita Growth Rate (%)"
  ) +
...
ggplot(low_growth2, aes(x = Year, y = average_gdp_growth, color = Continent, group = Continent)) +
  geom_line(linewidth = 1.25) +
  geom_hline(yintercept = 7, linetype = "dashed", color = "black", linewidth = 1) +
